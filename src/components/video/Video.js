...
import { Muted, VideoAlert } from "../../assets/Icons";
import {
  convertMinsSecs,
  LOADING_STATES,
...
class Video extends Component {
  ...
  volumeOutput = (value = null) => {
    ...
    if (!isNaN(value) === false) {
      ...
      self.setState({ volumeValue: String(newVolume), isVideoMuted: false });
  ...
  componentDidMount() {
    ...
    if (this.props.history.location.state && current_video_data) {
      ...
      if (comingFromMiniature && current_video_data) {
        ...
        if (current_video_data.all_formats.audioandvideoFormats.length === 0) {
          this.setState({ videoNotAvailable: true });
          return;
        }
  ...
  UNSAFE_componentWillReceiveProps(newProps) {
    ...
    if (newProps.query !== this.props.query) {
      this.addVideoToHistorial(this.props.current_video_data, this.props.user);
    }
  ...
  render() {
    ...
    return (
      <>
        <VideoPlayerContainer
          ...
          className={
            fullscreenActiveState
              ? videoContainerClass + " video-container--fullscreen"
              : videoContainerClass
          }
          onMouseOut={
            width > 768
        ...
        >
          ...
          {video
            ? !!window.chrome &&
              ...
              video.volume === 0 && (
                <MobileVolumeControl controlVolume={controlVolume} />
              )
            : null}
          ...
          {!videoNotAvailable && (
            <VideoControls
              ...
              openFullscreen={() => openFullscreen(videoContainer.current)}
        ...
        {current_video_data && !isThumbnail && !current_video_data.isError ? (
          <UploaderInfo
            thumbnail={
              videoThumb === undefined || videoThumb === null ? "" : videoThumb
            }
            uri={uri}
  ...
  putVideo = (response, current_video_data = null) => {
    ...
    if (response != null) {
      ...
      if (
        ...
        this.setState({ videoNotAvailable: true }, () => {
          document.body.style = "overflow:auto";
          localStorage.setItem("video_data", JSON.stringify(response.data));
        });
        return;
      }
      ...
      const video_source =
        current_video_data.all_formats.audioandvideoFormats[
          current_video_data.all_formats.audioandvideoFormats.length - 1
        ].url;
  ...
  addVideoToHistorial = async (current_video_data, user) => {
    ...
    const video_info = {
      ...
      video_duration: convertMinsSecs(video.duration * 1000, video.duration),
  ...
  videoListeners = () => {
    ...
    video.onloadedmetadata = () => {
      ...
      self.setState({ videoNotAvailable: false, videoIsGeoBlocked: false });
  ...
  return (
    ...
    >
      <div className="keyboard-interactions-container">
        ...
        >
          <span className="middlelayer-volume">{volumeValue + "%"}</span>
        </div>
...
const VideoTitle = (props) => {
  ...
  return (
    <div
      style={{
        opacity: !isFullScreenActive ? 0 : paused ? 1 : hideControls ? 0 : 1,
      }}
  ...
  return (
    <div className="video-blocked">
      ...
      <div className="video-blocked-info">
        <span className={"video-blocked-text-title"}>Video unavailable</span>
