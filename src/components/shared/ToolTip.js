 ...
 const ToolTip = (props) => {
    ...
    if(videoControl)
        classNameTooltip += ' videocontrol-tooltip';
    return (
        <div
            onMouseOver={() => setTooltipVisibe(true)}
            onMouseOut={() => setTooltipVisibe(false)}
            className={childrenIsAbsolute ? 'tooltip-container-absolute' : 'tooltip-container'}
            style={style}
        >
            {(tooltipVisible && show) &&
            <span style={toolTipStyle} className={classNameTooltip}>{message}</span>
            }
