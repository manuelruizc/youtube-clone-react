import { _getVideoSource, LOADING_STATES } from './helpers';
...
export const _playOnMiniPlayer = async (
    ...
    if(data.isError) {
        ...
        updateVideoThumb({uri, currentTime: length ? parseInt(length) : 0, videoURL: formats[1].url, thumbnail:true});
        const thumbVideo = document.getElementById("thumb-video");
        thumbVideo.src = response.data.formats[1].url;
        thumbVideo.currentTime = video.current_length ? 
                                        video.current_length === video.length ? 0 :
                                        video.current_length : 0;
    ...
    if(data.isError) {
        ...
        history.push({
          ...
          search: video.current_length ?
                    video.current_length === video.length ? `?v=${uri}` :
                    `?v=${uri}&t=${video.current_length}s` : `?v=${uri}`,
          state: { currentTime }
        });
        return;
    }
